...
the next burst below the active-Decode watermark. The upstream vLLM scheduler
...
class _PrefillAdmissionSchedulerMixin:
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        if decision.throttle_prefills:
            # Suppress upstream capacity-bound overrides during cooldown.
            self.prefill_capacity_bound = False

        deferred_prefills: list[tuple[object, list[Request]]] = []
        if not decision.throttle_prefills and decision.pending_prefill_ids:
            deferred_prefills = self._defer_non_cohort_prefills(
                decision.pending_prefill_ids
            )
